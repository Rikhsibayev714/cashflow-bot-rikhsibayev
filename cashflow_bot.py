...
#  НАСТРОЙКИ
# ─────────────────────────────────────────────
BOT_TOKEN      = os.environ.get("BOT_TOKEN", "7836899848:AAEe6E2kCQ-rKR6sjPbPPYY5HTtgym6UM5Y")
SPREADSHEET_ID = os.environ.get("SPREADSHEET_ID", "1zqIxt1oFRDW08mhuPXdxHpcmxOycCpY6I_wTV-ceFqM")
# На Railway credentials кладём в переменную окружения GOOGLE_CREDS (JSON строка)
# Локально — рядом с ботом файл credentials.json
...
def find_next_row(ws_cash) -> int:
    """Первая пустая строка в Cashflow (данные с row=3, row=1 заголовок, row=2 итоги)."""
    col_a = ws_cash.col_values(1)   # все значения столбца A
    col_b = ws_cash.col_values(2)
    # Ищем первую строку >= 3 где и A и B пустые
    max_len = max(len(col_a), len(col_b))
    for i in range(2, max_len):     # i=2 → row=3
        a = col_a[i] if i < len(col_a) else ""
        b = col_b[i] if i < len(col_b) else ""
        if not str(a).strip() and not str(b).strip():
            return i + 1            # gspread row = index+1
    return max_len + 1


def write_transaction(data: dict) -> tuple[bool, str]:
    try:
        ...
        ws_cash.update(f"A{row}", [[date_str]])
        ws_cash.update(f"B{row}", [[data["kassa"]]])
        ws_cash.update(f"C{row}", [[data["inflow_uzs"]  or ""]])
        ws_cash.update(f"D{row}", [[data["inflow_usd"]  or ""]])
        ws_cash.update(f"E{row}", [[data["note"]]])
        ws_cash.update(f"F{row}", [[data["outflow_uzs"] or ""]])
        ws_cash.update(f"G{row}", [[data["outflow_usd"] or ""]])
        ws_cash.update(f"T{row}", [["Telegram Bot"]])
...
def read_last_rows(n: int = 5) -> list[dict] | str:
    try:
        ws_cash, _ = get_sheets()
        all_rows = ws_cash.get_all_values()
        # Данные начинаются с индекса 2 (row=3)
        data_rows = []
...
def read_balance() -> tuple[list | None, str | None]:
    try:
        _, ws_bal = get_sheets()
        all_rows = ws_bal.get_all_values()
        items = []
        for row in all_rows[2:]:    # с row=3
            ...
            uzs_raw = row[2] if len(row) > 2 else ""
            usd_raw = row[3] if len(row) > 3 else ""
...
def row_to_text(r: dict, idx: int = None) -> str:
    prefix = f"{idx}. " if idx else ""
    has_in  = r.get("in_uzs")  or r.get("in_usd")
    has_out = r.get("out_uzs") or r.get("out_usd")
    direction = "ПРИХОД" if has_in else "РАСХОД"
